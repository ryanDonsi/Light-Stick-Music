...
class AutoTimelineGeneratorBeat_v4 : AutoTimelineGenerator, SectionAwareGenerator {
    ...
    override fun generateWithSections(
        ...
        Log.d(TAG, "v4 [PERF] start file=$fileName musicId=$musicId")
        ...
        Log.d(TAG, "v4 [PERF] decode+beat=${System.currentTimeMillis() - t0Decode}ms frames=${fullEnv.size} hopMs=$HOP_MS beatMs=${beatInfo.beatMs} beats=${beatInfo.beats.size}")
        ...
        Log.d(TAG, "v4 [PERF] build=${System.currentTimeMillis() - t0Build}ms frames=${frames.size}")
        Log.d(TAG, "v4 [PERF] total=${System.currentTimeMillis() - t0Total}ms  file=$fileName durationMs=$durationMs")
                ...
                if (coverGapMs <= longThMs) {
                    ...
                    while (fillT < firstBeat) {
                        ...
                        val fillRotateTransit = if (section.engine == FgEngine.ON_TRANSIT_ROTATE && isBalladMode)
                            ON_ROTATE_BALLAD_TRANSIT else 0
                        val fillPeriod = if (section.engine == FgEngine.STROBE) 1 else null
