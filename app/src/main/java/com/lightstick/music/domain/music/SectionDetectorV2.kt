...
/**
 * SectionDetectorV2 — SectionDetectorV1 + 비트 단위 특징 보강
 *
 * V1 대비 변경점은 annotateBeats() 하나뿐이다. 섹션 경계 판정/타입 분류/병합·컴팩션
 * 로직(buildFeatureWindows ~ applyIntroOutro)은 V1과 완전히 동일하게 유지한다 —
 * "섹션이 어디서 나뉘는가"는 여전히 2초 윈도우 기준의 거시적 판단이 맞고, 비트 단위로
 * 잘게 쪼개면 오히려 섹션이 노이즈에 흔들려 깜빡거리게 된다.
 *
