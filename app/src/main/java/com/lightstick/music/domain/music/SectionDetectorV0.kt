 ...
 */
class SectionDetectorV0 : SectionDetector {
    ...
    override fun detect(
        ...
        Log.d(TAG, "SectionDetectorV0 thresholds: lowTh=${"%.3f".format(lowTh)} highTh=${"%.3f".format(highTh)}")
    ...
    private fun toSections(windows: List<FeatureWindow>): List<SectionDetector.Section> =
        windows.mapIndexed { idx, s ->
            Log.d(TAG, "SectionDetectorV0[$idx] ${s.startMs}~${s.endMs} type=${s.sectionType} change=${s.changeStrength}")
    ...
    private fun detectClimaxMoments(
        ...
        if (cv < CLIMAX_MIN_CV || peakRatio < CLIMAX_MIN_PEAK_RATIO) {
            Log.d(TAG, "SectionDetectorV0 climax skip: CV=${"%.3f".format(cv)} peakRatio=${"%.2f".format(peakRatio)}")
            return emptyList()
        }
        Log.d(TAG, "SectionDetectorV0 climax CV=${"%.3f".format(cv)} peakRatio=${"%.2f".format(peakRatio)} → detecting")
