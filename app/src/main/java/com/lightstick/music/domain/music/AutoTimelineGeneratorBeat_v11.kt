 ...
 *
 * [PERF] 단일 패스 오디오 디코딩 (v8 동일)
 */
class AutoTimelineGeneratorBeat_v11 : AutoTimelineGenerator {
    ...
    companion object {
        ...
        private const val BEAT_BRIGHT_RATIO = 10L  // beatMs / 10 = dim 전환 시점
        private const val BEAT_FADE_DIM     = 60   // sustain 구간 fade 값
    ...
    override fun generate(
        ...
        Log.d(TAG, "v11 generate() start file=$musicPath musicId=$musicId paletteSize=$paletteSize")
        ...
        Log.d(TAG, "v11 frames(final)=${frames.size}")
        ...
        for ((index, section) in sections.withIndex()) {
            ...
            for ((beatIndex, t) in effectiveSectionBeats.withIndex()) {
                if (beatIndex == 0 && section.type == SectionType.INTRO) {
                    ...
                    // ON_TRANSIT_ROTATE: 매 비트 전송 / STROBE·BREATH: 중복 파라미터 차단
                    val skipRepeat = when {
                        effectiveBeatEngine == FgEngine.ON_TRANSIT_ROTATE -> { lastRepeatKey = null; false }
                        effectiveBeatEngine == FgEngine.STROBE || effectiveBeatEngine == FgEngine.BREATH -> {
                    ...
                    // ON_PULSE / ON_TRANSIT_ROTATE: 10% 구간 fade=100% → 나머지 fade=60%
                    if ((effectiveBeatEngine == FgEngine.ON_PULSE || effectiveBeatEngine == FgEngine.ON_TRANSIT_ROTATE)
                        && !skipRepeat) {
                        ...
                        if (dimT > t && !frameMap.containsKey(dimT)) {
                            putFrame(dimT, LSEffectPayload.Effects.on(color = fg, transit = ON_TRANSIT, fade = BEAT_FADE_DIM).toByteArray(),
                                section, "BEAT_DIM", effectiveBeatEngine, fg = fg, transit = ON_TRANSIT,
                                note = "beatIndex=$beatIndex fade60% at ${dimT - t}ms")
                        }
                    }
                }
            }
