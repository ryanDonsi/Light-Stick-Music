...
| **GTZAN F1 점수** | 85.6% | 88.5% | 70%(Ellis 알고리즘 기준) |
...
- **규칙적 비트에 강함**: HMM의 템포 일관성 유지가 K-POP 같은 정박자 음악에 유리 (실제 테스트: K-POP 87.3% vs BT 74.3%)
...
### BeatDetector에서의 활용
- **가중치**: 60% (신뢰도 기준)
- **추천 용도**: 규칙적 비트의 음악 분석 (K-POP, POP, Dance 등)
- **실제 성능**: K-POP 곡 분석 시 madmom 87.3% vs BT 74.3% (13%p 차이)
- **주의 사항**: CLASSICAL 장르에서는 madmom 이점이 거의 없음
...
### 단점 ❌
- **낮은 정확도**: 70%(Ellis 알고리즘 기준) (madmom 대비 15.6%p 낮음)
...
| madmom | 85.6% | 2위 | ⭐⭐⭐⭐⭐ |
| librosa | 70%(Ellis 알고리즘 기준) | 3위 | ⭐⭐⭐ |

### 처리 속도 비교 (이론상 추정치 - CPU 기준, 5분 곡)
| 엔진 | 처리 시간 | 상대 속도 | 실시간 여부 | 근거 |
|------|----------|---------|-----------|------|
| librosa | ~2초 | 1배 (기준) | ✓ 가능 | 간단한 알고리즘 |
| madmom | ~20초 | 10배 | ✗ 어려움 | CNN + HMM 아키텍처 |
| Beat Transformer | ~30초 | 15배 | ✗ 어려움 | 6 layers Transformer (GPU 권장) |

**주의**: 
- BeatDetector 프로젝트에서는 순수 GT엔진 처리 시간을 측정하지 않음
- 실제 처리 시간은 하드웨어(CPU/GPU), 음악 길이, 샘플링 레이트에 따라 크게 달라짐
- 위의 시간은 엔진 아키텍처 복잡도 기반의 **추정치**임

### BeatDetector 실제 테스트 결과 (V1 기준)
| 장르 | madmom | BT | 차이 | 추천 |
|------|--------|-----|------|------|
| **K-POP** | 87.3% | 74.3% | +13.0%p | madmom |
| **POP** | 69.0% | 65.2% | +3.8%p | madmom (약간) |
| **CLASSICAL** | 96.9% | 93.9% | +2.9%p | madmom (미미) |

**결론**: 
- K-POP에서 madmom의 명확한 우위 (13.0%p 차이)
- 이유: HMM 기반 템포 일관성 유지가 규칙적인 K-POP 비트에 유리
- madmom이 K-POP을 위해 특별히 설계된 것은 아님 (우연의 성능 우위)
- 다른 정박자 음악(POP, Dance)에서도 유사한 경향
